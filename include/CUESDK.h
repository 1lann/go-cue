...
#include "CorsairLedIdEnum.h"
	...
	typedef enum		// contains list of available device types
	{
		CDT_Unknown = 0,
		CDT_Mouse = 1,
		CDT_Keyboard = 2,
	...
	typedef enum	// contains list of available physical layouts for keyboards
	{
	...
	typedef enum	// contains list of available logical layouts for keyboards
	{
	...
	typedef enum		// contains list of device capabilities
	{
		CDC_None = 0,				// for devices that do not support any SDK functions
		CDC_Lighting = 1				// for devices that has controlled lighting
	} CorsairDeviceCaps;

	typedef enum		// contains list of available SDK access modes
	{
		CAM_ExclusiveLightingControl = 0
	} CorsairAccessMode;

	typedef enum			// contains shared list of all errors which could happen during calling of Corsair* functions
	{
	...
	typedef struct	// contains information about device
	{
	...
	typedef struct	// contains led id and position of led rectangle.Most of the keys are rectangular.In case if key is not rectangular(like Enter in ISO / UK layout) it returns the smallest rectangle that fully contains the key
	{
	...
	typedef struct	// contains number of leds and arrays with their positions
	{
	...
	typedef struct		//  contains information about led and its color
	{
	...
	typedef struct // contains information about SDK and CUE versions
	{
